...
lock	 mutex		  // 互斥锁
```
在创建channel时,在内存中实例化了一个hchan的结构体，并返回一个ch指针。channel是一个指针，指向一个hchan的实例。
```go
ch := make(chan int, 3)
```
在channel的接收与发送
...
sendx: 需要发送的元素的索引位置(index)，实际上是元素写入时存放到队列中的位置。
> 等待发送-没有数据，需要有数据写入才能发送出去

recvx: 需要接收的元素的索引位置(index)，实际上是元素从队列的该位置读出。
> 等待接收-有数据，需要有人过来取走数据
